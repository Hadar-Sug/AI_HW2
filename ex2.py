...
class OptimalPirateAgent:
        ...
        for act in action:
            ...
            for ship in curr_state['marine_ships']:
                if curr_state['pirate_ships'][act[1]]['location'] == \
                        ...
                        and curr_state['pirate_ships'][act[1]]['capacity'] == 0:
                    v -= 10
                # if len
    ...
    def next_states(self, state, action, treasure_options, marine_options):
        ...
        if action[0] == 'terminate':
            return []
        if action[0] == 'reset':
            reset_state = {'pirate_ships': self.initial['pirate_ships'], 'treasures': self.initial['treasures'],
    ...
    def get_ship_actions(self, state, ship_name, ship_info):
        ...
        if capacity == 0 and self.map[ship_x][ship_y] == 'B':
...
class PirateAgent:
    ...
    def act(self, state):
        ...
        for pirate_name in state['pirate_ships'].keys():
            if pirate_name != optimal_pirate:
                if self.ok:
                    actions.append(('wait', pirate_name))
                else:
                    if action[0] == 'sail':
                        if state['pirate_ships'][pirate_name]['location'] == action[2]:
                            actions.append('sail', pirate_name, (state['pirate_ships'][optimal_pirate]['location']))
                        else:
                            actions.append('wait')
                    else:
                        actions.append('wait')
...
class InfinitePirateAgent:
    ...
    #         # self.map =initial['map']
    # self.gamma = gamma
    # self.value_function = {state: 0 for state in self.build_all_states()}
    # self.policy = {}
    ...
    #                 value = self.calculate_state_value(state, action)
    #                 if value > max_value:
    #                     max_value = value
    ...
    #     ship_x, ship_y = ship_info["location"]
    #     capacity = ship_info['capacity']
    #     if self.map[ship_x][ship_y] == 'B' and capacity < 2:
    #         ship_actions.append(('deposit', ship_name))
    #
    #     for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
    #         new_x, new_y = ship_x + dx, ship_y + dy
    ...
    #
    # def calculate_state_value(self, state, action):
    #     # Implement this method to calculate the value of a state-action pair
    #     pass
    #
    # def get_actions(self, state):
    #     # Implement this method to generate all possible actions for a given state
    #     pass
    def build_all_states(self):
        # Define the build_all_states function based on the game rules and initial state
        pirates_list = []
